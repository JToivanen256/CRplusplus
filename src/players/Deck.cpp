...
Deck::Deck(const std::vector<std::shared_ptr<Card>>& cards) : cards_(cards) {
  if (cards.size() != 8) throw std::invalid_argument("Deck must contain exactly 8 cards.");
}

void Deck::addCard(const std::shared_ptr<Card>& card) {
  if (cards_.size() < 8) cards_.push_back(card);
  else throw std::invalid_argument("Deck cannot contain more than 8 cards.");
}
...
void Deck::playCard(const std::shared_ptr<Card>& card) {
  auto it = std::find(cards_.begin(), cards_.end(), card);
  if (it != cards_.end()) {
    auto played = *it;
    cards_.erase(it);
    cards_.push_back(played);
  } else {
    throw std::invalid_argument("Card not found in deck.");
  }
}

const std::vector<std::shared_ptr<Card>>& Deck::getCards() const { return cards_; }
...
std::vector<std::shared_ptr<Card>> Deck::getHand() const {
  return std::vector<std::shared_ptr<Card>>(cards_.begin(), cards_.begin() + 4);
}

std::vector<std::shared_ptr<Card>> Deck::initializedHand() {
  shuffle();
  return getHand();
}